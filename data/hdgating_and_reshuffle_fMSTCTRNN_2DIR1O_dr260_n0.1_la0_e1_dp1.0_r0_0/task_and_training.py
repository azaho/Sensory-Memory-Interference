...
hyperparameters.update({
    "random_string": str("0"),  # human-readable string used for random initialization (for reproducibility)
})
task_parameters.update({
    "task_name": "2DIR1O",
    "input_direction_units": 260,  # how many direction-selective input units?
    "dim_input": 260 + 1,  # plus one input for go cue signal
})
model_parameters.update({
    "model_name": "hdgating_and_reshuffle_fMSTCTRNN",
    "dim_recurrent": 260,
    "dim_input": 260 + 1,  # plus one input for go cue signal
})
additional_comments += [
...
#R1_pref_changes = [random.randint(-130, 130) for x in R1_pref_changes]
...
class Model(Model):
    def __init__(self):
        ...
        self.top_parameters = nn.Parameter(torch.tensor([3, -0.5, -0.5])/260*10)
